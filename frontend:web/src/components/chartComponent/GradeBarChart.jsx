...
const GradeBarChart = ({ data }) => {
  console.log("등급별 막대 그래프 데이터:", data);
  if (!data) return null;

  //총 아동 수 계산
  const totalGrades = data.reduce((sum, d) => sum + d.value, 0);
  ...
  return (
    <ChartContainer>
      {data.map((item, index) => {
        const percentage = totalGrades
          ? ((item.value / totalGrades) * 100).toFixed(1)
          : 0;
        return (
